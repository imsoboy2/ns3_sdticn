...
double throughput_C = 0;
double throughput_D = 0;
double throughput_E = 0;
double throughput_F = 0;
double throughput_G = 0;
double throughput_H = 0;
double throughput_I = 0;
double throughput_J = 0;
double throughput_K = 0;

int PktCount_Forwarding = 1;

const int numOfTier1Switch = 7;
const int numOfTier2Controller = 3;
const int E = 10;
...
void TestForwardingB (Ptr<Socket> socket, Ipv4Address srcaddr, int PktCount_Forwarding)
{
  ...
  std::cout << "SEND PACKET: src(" << srcaddr << ") dst(" << "10.1.1.5" << ", forwarding in Tier 1 & 2\n";
  ...
  while (received = socket->Recv ())
    {
      ...
      { // rule message
        ...
        if (hdr.GetDestination().Get() == ipAddr.Get()) {
          std::cout << "Forwarding Complete dest ip : " << hdr.GetDestination() << ", thisip: " << ipAddr << std::endl; 
        if (hdr.GetDestination() == "10.1.1.5")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_B += (bTotal) / 1000000.0; 
          ...
          else if (hdr.GetDestination() == "10.1.1.7")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_C += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.11")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now ;
            throughput_D += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.13")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_E += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.17")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_F += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.19")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now ;
            throughput_G += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.23")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_H += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.5")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_I += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.7")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_J += (bTotal) / 1000000.0;
          }
          else if (hdr.GetDestination() == "10.1.1.11")
          {
            int packets_now = received->GetSize();
            int bTotal = packets_now;
            throughput_K += (bTotal) / 1000000.0;
          }
          return;
